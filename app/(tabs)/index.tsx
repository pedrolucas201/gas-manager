  ...
  return (
    ...
    >
      <View className="bg-primary-500 px-5 pt-14 pb-6">
        ...
        <Text className="text-white text-2xl font-bold">Gas Manager</Text>
      </View>

      <View className="px-4 -mt-4 mb-4">
        <TouchableOpacity
          className="bg-white rounded-2xl p-4 flex-row items-center shadow border border-gray-100"
        ...
        >
          ...
          <View className="flex-1">
            <Text className="font-bold text-gray-900">Nova Venda</Text>
            <Text className="text-xs text-gray-400">Registrar venda de botijão</Text>
          </View>
          <Ionicons name="chevron-forward" size={18} color="#9ca3af" />
        </TouchableOpacity>
      </View>

      <Text className="px-4 text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Hoje</Text>
      ...
      <Text className="px-4 text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Estoque</Text>
      <View className="px-4 mb-4 gap-2">
        {inventory.map((item) => (
          <View key={item.id} className="bg-white rounded-xl px-4 py-3 flex-row items-center border border-gray-100">
            <View className="flex-1">
              <Text className="font-bold text-gray-900">{item.cylinder_name}</Text>
            ...
            <View
              className={`rounded-full px-3 py-1 ${
                item.full_qty <= 3 ? "bg-red-100" : item.full_qty <= 8 ? "bg-yellow-100" : "bg-green-100"
              }`}
            >
              <Text
                className={`text-xs font-bold ${
                  item.full_qty <= 3 ? "text-red-600" : item.full_qty <= 8 ? "text-yellow-700" : "text-green-700"
                }`}
              >
      ...
      {debtors.length > 0 && (
        <>
          <Text className="px-4 text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Fiado</Text>
          <TouchableOpacity
            className="mx-4 mb-6 bg-red-50 border border-red-200 rounded-2xl p-4 flex-row items-center"
            onPress={() => router.push("/(tabs)/customers")}
          >
            <Ionicons name="warning" size={22} color="#dc2626" />
            <View className="ml-3 flex-1">
              <Text className="font-bold text-red-700">{debtors.length} cliente(s) com fiado</Text>
              <Text className="text-xs text-red-500">Total devido: {formatCurrency(totalDebt)}</Text>
            </View>
