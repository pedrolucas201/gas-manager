  ...
  return (
    <ScrollView className="flex-1 bg-gray-50" keyboardShouldPersistTaps="handled">
      <View className="px-4 pt-4 gap-4">
            ...
            {cylinders.map((c) => (
              <TouchableOpacity
                ...
                className={`flex-1 min-w-[80px] rounded-xl p-3 border items-center ${
                  selected?.id === c.id ? "bg-primary-500 border-primary-500" : "bg-white border-gray-200"
                }`}
                onPress={() => handleSelectCylinder(c)}
              >
                <Text className={`font-bold text-base ${selected?.id === c.id ? "text-white" : "text-gray-900"}`}>
                  {c.name}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        <View>
          <Text className="text-sm font-semibold text-gray-700 mb-2">Custo por unidade (R$)</Text>
          <TextInput
        ...
        <View>
          <Text className="text-sm font-semibold text-gray-700 mb-2">Observações (opcional)</Text>
          <TextInput
